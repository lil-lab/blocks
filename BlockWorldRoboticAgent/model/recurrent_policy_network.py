...
class RecurrentPolicyNetwork:
    ...
    over actions for a given observed state s. Key distinguishing feature
    is that the policy uses recurrence over all past image observations. """

    def __init__(self, image_dim, num_actions, time_horizon):
        ...
        self.n_image = 200
        self.image_history_embedder = embed_image_recurrent.EmbedImageRecurrent(
            self.n_image, image_dim, self.n_image, time_horizon)
        image_embedding = self.image_history_embedder.get_output()
        ...
        self.n_previous_action_embedding = self.n_status_flag_dim + self.n_direction_dim + self.n_blocks_dim
        self.null_previous_action = (2, 5, 20)
        self.previous_action_embedder = epa.EmbedPreviousAction(
            3, self.n_status_flag_dim, 6, self.n_direction_dim, 21, self.n_blocks_dim)
    ...
    def evaluate_qfunction(self, image_history, text_input_word_indices, text_mask, previous_action, sess):
        ...
        standardized_image_history = []
        for image in image_history:
            standardized_image_history.append(self.image_preprocessor.get_standardized_image(image, sess))

        image_placeholder, mask_image_placeholder = self.image_history_embedder.get_images_data_and_mask()
        image_mask_input = self.image_history_embedder.get_mask_for_num_frames(len(image_history))

        text_input = text_embedder_bucket.get_input()
        text_mask = text_embedder_bucket.get_zero_mask()
        ...
        result = sess.run([block_prob, direction_prob],
                          feed_dict={text_input: text_input_word_indices,
                                     text_mask: text_mask, batch_size: 1,
                                     image_placeholder: [standardized_image_history],
                                     mask_image_placeholder: [image_mask_input],
